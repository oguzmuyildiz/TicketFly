...
public class UpdateAppNotificationCommandHandler(ILogger<UpdateAppNotificationCommandHandler> logger, IAppDbContext context) : IRequestHandler<UpdateAppNotificationCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(UpdateAppNotificationCommand request, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("AppNotification updated with ID: {Id}", entity.Id);
