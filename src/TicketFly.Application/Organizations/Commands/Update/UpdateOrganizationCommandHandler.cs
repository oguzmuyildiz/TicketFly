...
public class UpdateOrganizationCommandHandler(ILogger logger, IAppDbContext context) : IRequestHandler<UpdateOrganizationCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(UpdateOrganizationCommand request, CancellationToken cancellationToken)
    {
        ...
        logger.Information("Organization updated with ID: {Id}", entity.Id);
