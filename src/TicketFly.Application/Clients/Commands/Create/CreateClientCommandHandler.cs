...
public class CreateClientCommandHandler(ILogger logger, IAppDbContext context) : IRequestHandler<CreateClientCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateClientCommand request, CancellationToken cancellationToken)
    {
        ...
        logger.Information("Client created: {ClientId} - {ClientEmail}", entity.Id, entity.Email);
        return entity.Id;
    }
}
