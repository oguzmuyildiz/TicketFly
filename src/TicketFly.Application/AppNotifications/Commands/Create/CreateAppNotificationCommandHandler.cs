...
public class CreateAppNotificationCommandHandler(ILogger logger, IAppDbContext context) : IRequestHandler<CreateAppNotificationCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateAppNotificationCommand request, CancellationToken cancellationToken)
    {
        ...
        logger.Information("AppNotification created with ID: {Id}", entity.Id);
